...
const sections = [
    ...
    content: (
      <>
        ...
        <div className="my-7 rounded-2xl bg-black p-7 text-white sm:p-9">
          ...
          <p className="mt-2 text-sm leading-6 text-gray-200">Le test ATS est accessible sans connexion à un compte. Les données personnelles et le CV transmis pour réaliser ce test ne sont pas conservés à l’issue de l’analyse et ne sont jamais rendus publics.</p>
        </div>
      </>
    ),
  },
    ...
    content: (
      <div className="overflow-x-auto">
        <table className="w-full min-w-[38rem] border-collapse text-left text-sm">
          <thead><tr className="border-b-2 border-black text-black"><th className="py-3 pr-5 font-black">Finalité</th><th className="py-3 font-black">Base légale</th></tr></thead>
          <tbody>
            <tr className="border-b border-black/10"><td className="py-4 pr-5">Gestion du compte et authentification sécurisée</td><td className="py-4">Exécution du contrat / CGU</td></tr>
            <tr className="border-b border-black/10"><td className="py-4 pr-5">Recommandations d’offres et analyse assistée par IA</td><td className="py-4">Intérêt légitime / consentement, selon le traitement</td></tr>
            <tr className="border-b border-black/10"><td className="py-4 pr-5">Sauvegarde des favoris, compétences et CV</td><td className="py-4">Exécution du contrat / CGU</td></tr>
            <tr className="border-b border-black/10"><td className="py-4 pr-5">Statistiques anonymisées sur le marché de l’emploi</td><td className="py-4">Intérêt légitime</td></tr>
            <tr><td className="py-4 pr-5">Modération, prévention des abus et sécurité</td><td className="py-4">Intérêt légitime / obligation légale</td></tr>
          </tbody>
        </table>
      </div>
    ),
  ...
  },
  {
    ...
    label: 'Destinataires et partage',
    content: (
      <ul className={listClass}>
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <p>Pour exercer vos droits, écrivez à <a className="font-bold text-black underline decoration-[#d2915c] decoration-2 underline-offset-4 hover:text-[#8b4c25]" href="mailto:contact@overkill.com">contact@overkill.com</a>. Une réclamation peut également être adressée à la CNIL.</p>
      </>
    ),
  },
  {
...
function Privacy() {
  ...
  const tocLinks = sections.map((section) => (
    <a key={section.id} href={`#${section.id}`} aria-current={activeSection === section.id ? 'location' : undefined} onClick={() => setActiveSection(section.id)} className={tocClass(section.id)}>
      {section.number}. {section.label || section.title}
    </a>
  ))
        ...
        <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-12 lg:grid-cols-[250px_minmax(0,1fr)] lg:gap-16">
            ...
            <article className="min-w-0">
              <p className="mb-10 max-w-3xl text-xl font-medium leading-8 text-gray-700">OVERKILL traite les données à caractère personnel de ses utilisateurs conformément au règlement général sur la protection des données (RGPD — règlement (UE) 2016/679) et à la loi Informatique et Libertés.</p>
